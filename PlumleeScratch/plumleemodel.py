...
def emupred(thetanew, emumodel, returnvar = False):
    predvec = np.zeros((thetanew.shape[0],len(emumodel)))
    if returnvar is True:
    ...
    if returnvar is False:
        return predvec
    else: 
        return predvec, predvar

def emubuild(theta, obsmat):
    nummodel = obsmat.shape[1]
    ...
    for pcanum in range(0,nummodel):
        ...
        emuinfo['f'] =  1*obsmat[:nhyptrain,pcanum]
        ...
        W, V = np.linalg.eigh(R)
        fspin = V.T @ obsmat[:,pcanum]
        onespin = V.T @ np.ones(obsmat[:,pcanum].shape)
        ...
        modellist[pcanum]['pw'] = 1*Rinv @ (obsmat[:,pcanum] - muhat)
