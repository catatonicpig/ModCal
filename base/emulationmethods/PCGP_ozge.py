...
def fit(fitinfo, x, theta, f, args=None):
    ...
    fitinfo : Dictionary
        DESCRIPTION.
    x : Array
        An m-by-p array of inputs. Each column should correspond to a row in f.
    theta : TYPE
...
def predict(predinfo, fitinfo, x, theta, args=None):
    ...
    fitinfo : Dictionary
        A dictionary containing important fitting information from the fit function above.
    x : Array
        An m-by-p array of inputs.
    theta : Array
...
def emulation_covmat(theta1, theta2, gammav, returndir = False):
    ...
    -------
    Array
        A n1-by-n2 array of covariance between theta1 and theta2 given parameter gammav.
    ...
    if returndir:
        dR = np.zeros([n1, n2, d])
    for k in range(0, d):
        S = np.abs(np.subtract.outer(theta1[:, k], theta2[:, k])/np.exp(gammav[k]))
        R *= (1 + S)
        V -= S
        if returndir:
            dR[:, :, k] = (S * S) / (1 + S)
...
def emulation_negloglik(hyperparameters, fitinfo):
    ...
    negloglik += 1/2 * np.sum((hyperparameters - fitinfo['hypregmean']) **2 /(fitinfo['hypregstd'] ** 2))
    return negloglik
...
def emulation_negloglikgrad(hyperparameters, fitinfo):
    ...
    dnegloglik += ((hyperparameters - fitinfo['hypregmean'])  /(fitinfo['hypregstd'] ** 2))
    return dnegloglik
...
def emulation_fit(theta, pcaval, hypstarts=None, hypinds=None):
    ...
    # why, why, why? MAKE THAT PART ARGS
    ...
    opval = spo.minimize(emulation_negloglik, np.append(covhyp0, nughyp0), bounds = bounds, args=(subinfo), method='L-BFGS-B', 
                         options={'disp': False}, jac=emulation_negloglikgrad)
