...
def plotpreds(axis, pred):
    for k in (25,50):
        ...
        for l in range(0,1000):
            axis.plot(xtotv[inds,0],pred.info['modelrnd'][l, inds],'k-', alpha=0.01,linewidth=0.1)
        uppercurve = np.quantile(pred.info['modelrnd'][:, inds],0.975,0)
        lowercurve = np.quantile(pred.info['modelrnd'][:, inds],0.025,0)
